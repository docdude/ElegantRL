...
def run(gpu_id: int = 0, eval_only: bool = False, checkpoint: str = None,
    ...
    # ElegantRL's StockTradingEnv already applies reward_scale=2^-12 internally,
    # and the agent multiplies rewards *= reward_scale again in explore_env.
    # Adding VecNormalize norm_reward on top creates double-normalization that
    # causes divergence for BOTH on-policy (GAE noise amplification) and
    # off-policy (critic loss explosion). Empirically confirmed:
    #   - TD3 no VecNorm: objC stable at ~1.0 over 496K steps
    ...
    if use_vec_normalize:
        print(f"   📊 VecNormalize: norm_obs=True, norm_reward=False (env already scales rewards via reward_scale=2^-12)")
