...
# ── Paths ────────────────────────────────────────────────────────────────────
# PIPELINE_DIR = wyckoff_rl/data_pipeline/
# PROJECT_ROOT = ElegantRL repo root
# DATA_DIR     = wyckoff_rl/data/   (SCID input + NPZ output)

PIPELINE_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.dirname(os.path.dirname(PIPELINE_DIR))  # ElegantRL/
DATA_DIR = os.path.join(os.path.dirname(PIPELINE_DIR), "data")
OUTPUT_DIR = DATA_DIR
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Default SCID path — drop your .scid file into wyckoff_rl/data/
...
RANGE_BAR_SIZE = 4.0  # NQ points per bar (adjustable)
